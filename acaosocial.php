<?php include("header.php"); ?>

<link rel="stylesheet" href="css/pagina_entrada.css">

<div class="top-div-entrada">
	<div class="banner">
		<img src="images/banner_acaosocial.jpg">
	</div>
	<div class="descricao-box-entrada">
		<div class="descricao-box-texto-entrada">
			<h5 align="middle">AÇÃO SOCIAL</h5>
			<p style="font-style: italic">
				“Se houver algum israelita pobre em qualquer das cidades
				da terra que o Senhor, o seu Deus, lhe está dando, não
				endureçam o coração, nem fechem a mão para com o seu
				irmão pobre.” Dt. 15.7
			</p>
			<p>
				O Ministério de ação social tem como missão fundamental
				suprir as pessoas em suas necessidades essenciais. Tais
				necessidades, a nosso ver, são aquelas que nos dão uma
				certeza de que não estamos sozinhos nesta vida. Desse
				modo, temos basicamente duas necessidades essenciais:
				as espirituais e as materiais.
			</p>
			<p>
				As necessidades espirituais se
				traduzem no alvo definitivo da nossa vida, no conhecimento
				de que somos eternos e de que esta eternidade se encontra
				exclusivamente em Jesus Cristo.
			</p>
		</div>    
	</div>
</div>

<div class="top-div-informativa">
	<div class="banner-informativa">
		<img src="images/cestadoamor.jpg">

		<div class="descricao-box-informativa">
			<div class="descricao-box-texto-informativa">
				<h5 align="middle">CESTA DO AMOR</h5>				
				<p>
					A ação social é um ministério que ajuda mensalmente algumas
					famílias necessitadas com uma cesta básica de alimentos, a
					qual chamamos de cesta do amor. 
				</p>					
				<p>
					É uma forma de demonstrar o amor que Represa Nova tem e
					o cuidado com relação às famílias necessitadas. Essa sexta é
					feita a partir de doações de alimentos e doações em espécie.
					Estamos orando para que Deus levante mais recursos para
					ajudar mais famílias, porque até o momento, não é possível
					cadastrar mais pessoas. 
				</p>
				<p>
					Ore, doe, faça parte desse ministério de cuidado; o Senhor
					nos deixou também a missão de cuidar das necessidades dos
					nossos irmãos
				</p>				
			</div>
		</div>
	</div>

	<div class="banner-informativa">
		<img src="images/amoragape.jpg">	
		
		<div class="descricao-box-texto-informativa">
			<h5 align="middle">AMOR ÁGAPE</h5>
			<p>
				O ministério de ação social tem como missão demonstrar amor
				incondicional aos necessitados. Independentemente de você
				fazer parte da membresia da igreja ou de como se encontra
				sua situação espiritual, entendemos que o amor ao próximo
				precisa observar necessidades físicas e espirituais. 
			</p>
			<p>
				Nós buscamos demonstrar o amor agape – o amor
				incondicional que Cristo demonstrou na cruz, morrendo
				por nós apesar de sermos pecadores e não merecedores da
				graça. Ele morreu em nosso lugar para nos salvar. A mão que
				podemos estender ao outro é em resposta ao grande amor
				com que Ele nos amou.
			</p>
			<p style="font-style: italic">
				Mas Deus demonstra seu amor por nós: Cristo morreu em
				nosso favor quando ainda éramos pecadores. Romanos 5.8
			</p>
		</div>
		
	</div>

	<div class="banner-informativa">
		<img src="images/comoajudar.jpg">	
		
		<div class="descricao-box-texto-informativa">
			<h5 align="middle">COMO AJUDAR?</h5>
			<p>
				Você que tem sido abençoado por Deus em suas necessidades,
				te convidamos a unir forças conosco para suprir necessidades
				de quem passa por dificuldades materiais e espirituais. A ajuda
				material depende da sua disposição de separar tempo em sua
				vida para o atendimento aos necessitados.
			</p>
			<p>
				Além da sua ajuda pessoal, necessitamos de ofertas para
				aquisição dos produtos que serão distribuídos. Se preferir, você
				pode ofertar alimentos que comporão as cestas básicas, cujos
				alimentos precisamos todos os meses, tais como: arroz, feijão,
				açúcar, sal, farinha de trigo, farinha de mandioca, fubá, óleo,
				macarrão, molho de tomate, café, leite, bolachas (salgadas e
				doces), queijo ralado, etc.
			</p>
			<p>
				A ajuda espiritual não é menos importante, precisamos das
				suas orações em favor dos necessitados e daqueles que ainda
				não conhecem Jesus.
			</p>				
		</div>
		
	</div>
</div>

<?php include("footer.php"); ?>